import type { ATSAnalysis, Recommendation, KeywordAnalysis, GrammarIssue } from '@/types/analysis';
    ...
    try {
      const response = await this.makeRequest({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert ATS (Applicant Tracking System) analyzer. Provide detailed, actionable feedback on resume optimization for ATS systems. Always respond with valid JSON only.'
          },
          {
            role: 'user',
            content: prompt
...
5. ${jobDescription ? 'Alignment with job requirements' : 'General best practices'}
...
Provide specific, actionable recommendations for improvement.`;
  ...
  private parseAnalysisResponse(response: OpenRouterResponse): ATSAnalysis {
    try {
      ...
      const analysis: ATSAnalysis = {
        ...
        breakdown: {
          formatting: Math.max(0, Math.min(100, parsed.breakdown?.formatting || 0)),
          keywords: Math.max(0, Math.min(100, parsed.breakdown?.keywords || 0)),
          structure: Math.max(0, Math.min(100, parsed.breakdown?.structure || 0)),
  ...
  private validateRecommendations(recommendations: any[]): Recommendation[] {
    return recommendations
      ...
      .map((rec, index) => ({
        id: rec.id || `rec-${index}`,
        category: ['formatting', 'content', 'keywords', 'structure'].includes(rec.category) 
          ? rec.category 
          : 'content',
   ...
   */
  private validateKeywordMatch(keywordMatch: any): KeywordAnalysis {
    return {
      ...
      missing: Array.isArray(keywordMatch.missing) 
        ...
        : [],
      matchPercentage: Math.max(0, Math.min(100, keywordMatch.matchPercentage || 0)),
      density: Math.max(0, keywordMatch.density || 0),
      suggestions: Array.isArray(keywordMatch.suggestions) 
        ? keywordMatch.suggestions.filter(s => typeof s === 'string').slice(0, 10)
        : [],
    };
  }
